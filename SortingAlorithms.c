...
// O segundo arr[m+1..r] que indica a segunda metade do meio + 1 ao fim (direita)
...
void mergeFinal(int array[], size)
{
  ...
  *array = malloc(2000000 * sizeof(*array)); // mallocando
  mergeSort(array, 0, size - 1); //executa função principal que executará as outras duas auxiliáres
  t = clock() - t; //finaliza contagem
  double time_taken = ((double)t)/CLOCKS_PER_SEC; //em segundos
